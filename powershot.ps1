<#
    ...
    .DESCRIPTION
        クリップボードの画像データを監視し、スクリーンショットを行うとファイル保存画面と取得した画像をを表示します。
		...
		停止する場合はコンソールでCtrl-Cを押下してください。
		このスクリプトはPowerShell 5.1でのみ動作確認しています。
    ...
    .EXAMPLE
		powershot.batを実行するか、PowerShellコンソールからpowershot.ps1を実行してください。
...
Function saveSS($FileName) {
	...
	# ファイル名に拡張子を付与
	$fileEncode = ($FileName) + ".png"

	# フルパスにする
	$imagePath = Join-Path $saveDir $fileEncode
