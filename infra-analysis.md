...
- **起動方法:** `start_analysis.bat` 等のバッチファイルを実行すると、ローカルで Streamlit サーバーが起動し、自動的にユーザーのデフォルトブラウザでホーム画面（`http://localhost:8501`）が開く形式とする。
  Python のみで高速にデータ分析 UI を構築でき、DuckDB や Pandas との親和性が非常に高い。
- **ナビゲーション:** Streamlit のサイドバーから各機能（分析画面、設定画面）にアクセスできる。また、ホーム画面からも主要機能へのクイックアクセスを提供する。
- **初回起動時の処理:** `config.yml` が存在しない場合、初回セットアップ画面を表示し、ユーザーは以下のいずれかを選択できる:
  ...
  - **セットアップをスキップして閲覧モードで開始:**
    - データ収集や設定変更機能は利用できないが、既存の計測データの参照・分析機能のみ利用可能な「閲覧モード」として起動する
...
└── app/                       # ツール本体は app/ 以下へまとめて格納
    ...
    │       ├── data_buffer.py     # データバッファ管理
    │       ├── data_processor.py  # データ処理・変換（SNMP/vSphere）
    │       ├── parquet_writer.py  # Parquet ファイル書き込み
    │       ├── credential_manager.py  # 認証情報管理（Windows Credential Manager）
    │       └── logger_config.py   # ログ設定・ローテーション
    ├── maintenance/               # メンテナンスジョブ
    │   └── maintenance_job.py     # マージ・リテンション処理
    ├── ui/                        # Streamlit UI
    │   ├── app.py                 # メインアプリケーション（ホーム画面）
    │   ├── pages/                 # 各ページ
    │   │   ├── setup.py           # 初回セットアップ画面
    │   │   ├── analysis.py        # 分析画面
    │   │   └── settings.py        # 設定画面
    │   └── backend/               # バックエンドロジック
    │       ├── query_engine.py    # DuckDB クエリ生成・実行
    │       ├── baseline_analyzer.py  # ベースライン分析
    │       ├── graph_generator.py    # グラフ生成（折れ線、散布図、ヒートマップ）
    │       ├── summary_generator.py  # 統計サマリ表生成
    │       ├── report_generator.py   # レポート生成（CSV/PDF）
    │       ├── config_manager.py     # 設定ファイル管理（config.yml）
    │       ├── export_manager.py     # データエクスポート（ZIP）
    │       ├── system_manager.py     # Windows サービス・タスクスケジューラ制御
    │       ├── vsphere_inventory.py  # vSphere インベントリ取得・キャッシュ
    │       └── snmp_tester.py        # SNMP OID テスト機能
    ├── config/                    # 設定ファイル
    │   ├── config.yml
...
`app.py` がホーム画面として機能し、以下の情報を表示する。
...
- **ツールの概要・説明:**
  ...
  - ツールの目的と設計思想の簡潔な説明
  - 主要機能（ベースライン分析、シンプルな閲覧機能）の案内

- **システム状態の表示:**

  - **コレクターの状態:**
    - SNMP コレクター（Windows サービス）の状態（実行中/停止中）
    - vSphere コレクター（タスクスケジューラ）の状態（有効/無効、実行状態）
    - メンテナンスジョブ（タスクスケジューラ）の状態
  - **データ収集の遅延警告:**
    - 最終書き込み完了時刻が現在時刻から著しく（例: 1 時間以上）遅れている場合、「**データ収集中または遅延の可能性があります**」という警告メッセージを表示する（3.4.4 節参照）

- **データストレージの状態:**

  - 利用可能なデータ期間（最古の日付 〜 最新の日付）
  - 最新データの日時
  - ストレージ使用量（設定されている上限に対する使用率）

- **閲覧モードの場合の案内:**

  - `config.yml` が存在しない場合、閲覧モードである旨を表示
  - フル機能を有効化するための案内（設定画面へのリンク）

- **各機能へのクイックアクセス:**
  - サイドバーでもアクセス可能だが、ホーム画面からも主要機能への案内を表示
    - 分析画面へのリンク
    - 設定画面へのリンク

#### 6.2. 分析画面

ユーザーが分析対象を選択後、グラフや統計サマリを表示する機能。分析対象の選択前はグラフを表示しない。
...
- **UI カテゴリ連携:**
  ...
  - SNMP データの場合: `WHERE host IN (...)` でクエリ。
...
分析対象を選択後、各分析カテゴリに応じて以下のグラフパターンを自動的に生成する。メトリクスは、カテゴリ（CPU、メモリ、ディスク、ネットワーク）ごとに別のグラフとして表示する。同一カテゴリ内のメトリクス（例: CPU Usage と CPU Ready）は同じグラフに表示する。
...
分析対象を選択後、すべての分析カテゴリにおいて、統計サマリ表を出力する。統計サマリ表は以下の 2 種類を出力する:

- **シンプルな統計情報の一覧:** 対象（ホスト、VM、インターフェース等）、メトリクス名、平均値、最大値、最小値、中央値、95 パーセンタイル値を記載する。これにより、数値的な比較が可能となる。
- **ベースライン分析の結果:** ベースライン分析（6.3 節参照）により検出された異常値の期間を記載する。これにより、過去に発生した性能スパイクや予期せぬ落ち込みが「いつ発生したか」を把握できる。
...
- **PDF:** 分析サマリ表（統計表）およびグラフデータを PDF として生成する。
  分析画面での閲覧とは別に、オフラインでの報告や共有のために、固定フォーマットの出力機能が必要。
...
- **UI 警告表示:**
  - VM 単位のデータ取得設定画面（または分析画面）において、「**VM の Memory, Disk, Network メトリクスは、vCenter への負荷およびデータ処理遅延の原因となるため、必要に応じて対象を絞って有効化してください。**」という警告を表示するためのフラグ/機能を提供する。
