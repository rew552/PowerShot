...
**設計思想:** 本ツールは、性能分析の専門知識がなくても、ある程度ボトルネックや性能に関する問題を特定できることを重視した設計とする。UI は、複雑な統計知識やパフォーマンス指標の解釈方法を知らなくても、直感的に問題箇所を把握できるよう、視覚的な表示や自動判定機能を充実させる。ボトルネック分析機能（6.3 節参照）では、定義済みのルールに基づいて自動的に問題を検出し、ユーザーに分かりやすく提示する。
...
- **除外対象:**
  - **vCLS（vSphere Cluster Services）VM:** vCLS は vSphere の管理用 VM であり、ユーザーが管理するワークロードではないため、収集対象から除外する。VM 名に `vCLS-` プレフィックスが含まれる VM を自動的に除外する。
  - **停止している VM:** 電源がオフ（`poweredOff`）の VM は、パフォーマンスデータが取得できないため、収集対象から除外する。VM の電源状態を `VirtualMachine.runtime.powerState` プロパティで確認し、`poweredOff` の場合はスキップする。
...
**UI 設計思想:** 本機能は、性能分析の専門知識がなくても問題を特定できるよう、以下の設計方針に基づいて実装する。

- **自動判定:** 統計計算は定義済み手法（例: パーセンタイル、中央値等）を適用し、ボトルネック判定の主要なロジックは自動的に実行するが、「警告」とする各メトリクスの閾値はユーザーが設定できるものとする（デフォルト値も用意）。
- **視覚的な提示:** ボトルネックが検出された場合、どのリソースが問題かを色分けやアイコンで直感的に表示する（例: CPU ボトルネックは赤、ディスクボトルネックはオレンジ）。
...
- **判定ロジック:**
  ...
  - 各リソースについて、上記の 3 つの観点で評価し、警告が発生した数に応じてボトルネック度を判定する。
    - 警告が 1 つの場合：「ボトルネック度：中」とする。
    - 警告が 2 つ以上の場合：「ボトルネック度：大」とする。
  - 複数のリソースでボトルネックが検出された場合、優先順位は以下の通り:
    1. CPU（最も重要）
    2. ディスク
    3. メモリ
    4. ネットワーク
